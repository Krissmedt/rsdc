...
for key,value in filenames.items():
    ...
    gamma_sol = conf.gamma(times[-1])
    x_sol = conf.ref_x(times[-1])
    v_sol = conf.ref_vx(times[-1])

    ref_errors = np.abs(x_sol-x[:,:,0])/np.abs(x_sol)
    ref_errors = np.linalg.norm(ref_errors,axis=1)

    refv_errors = np.abs(v_sol-v[:,:,0])/np.abs(v_sol)
