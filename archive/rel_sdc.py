...
from gammafac import G, gu
from fields import E,B,F
from boris import boris_daniel
from gauss_legendre import CollGaussLegendre
from gauss_lobatto import CollGaussLobatto

def boris_SDC(pos,vel,coll,fields):
    E = fields.E
    B = fields.B
    F = fields.F
    ...
    coll.x[0,:,:] = pos
    coll.u[0,:,:] = vel
    coll.F[0,:,:] = F(vel,E(pos,q=qe),B(pos,q=qe))
    ...
    ###### Initial Step #########################
    v_half = vel + 0.5*dm[0]*F(vel,E(pos,q=qe),B(pos,q=qe))
    ...
    coll.F[1,:,:] = F(coll.u[0,:,:],E(coll.x[0,:,:]),B(coll.x[0,:,:]))
    ...
    for m in range(1,M):
        ...
        coll.F[m+1,:,:] = F(coll.u[m+1,:,:],E(coll.x[m+1,:,:]),B(coll.x[m+1,:,:]))
    ...
    for k in range(1,K+1):
        ...
        coll.Fn[1,:,:] = F(coll.un[1,:,:],E(coll.xn[1,:,:]),B(coll.xn[1,:,:]))
        ...
        for m in range(coll.ssi,M):
            ...
            coll.Fn[m+1,:,:] = F(coll.un[m+1,:,:],E(coll.xn[m+1,:,:]),B(coll.xn[m+1,:,:]))
